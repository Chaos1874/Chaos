...
public class ImageDownLoader {
    private static final String ImageDownLoader_Log = FileUtils
            .makeLogTag(ImageDownLoader.class);

    ArrayList<Callable<Bitmap>> callers = new ArrayList<Callable<Bitmap>>();
    ...
    private File cacheFileDir;//---------------------------------------------初始化失败---------------------------------------------------------
    /** 缓存文件夹 */
    private static final String DIR_CACHE = "caches";
    /** 缓存文件夹最大容量限制（10M） */
    private static final long DIR_CACHE_LIMIT = 10 * 1024 * 1024;
    /** 图片下载失败重试次数 */
    private static final int IMAGE_DOWNLOAD_FAIL_TIMES = 2;

    private Bitmap bitmap;//bitmap临时容器

    public ImageDownLoader(Context context) {//构造方法，初始化要用的对象
        // 获取系统分配给每个应用程序的最大内存
    ...
    /**     * 添加Bitmap到内存缓存     *      * @param key     * @param bitmap     */
    ...
    /**     * 从内存缓存中获取Bitmap     *      * @param key     * @return     */
    ...
    /**     * 获取Bitmap, 若内存缓存为空，则去文件缓存中获取     *      * @param url     * @return 若缓存中没找到，则返回null     */
    ...
    /**     * 下载图片，并按指定高度和宽度压缩     *      * @param url     * @param width     * @param height     * @return     */
    private Bitmap downloadImage(String url, int width, int height) {
        Bitmap bitmap = null;
        try {
                HttpURLConnection connection = HttpConnettionUtils.getpicConnection(url);
                //解决缩放大图时出现SkImageDecoder::Factory returned null错误
                byte[] byteIn =StreamString.InputStreamToByte(connection.getInputStream());
